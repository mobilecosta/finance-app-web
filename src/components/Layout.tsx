...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-zinc-950">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-zinc-900 border-r border-zinc-800 transition-all duration-300 flex flex-col`}
      >
        {/* Logo */}
        ...
        <div className="p-4 border-t border-zinc-800 space-y-3">
          {sidebarOpen && (
            <div className="px-2">
              <p className="text-xs text-zinc-500 uppercase tracking-wider font-medium">Usuário</p>
              <p className="text-sm font-medium text-zinc-100 truncate">{user?.email}</p>
            </div>
          )}
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-all duration-200"
          >
