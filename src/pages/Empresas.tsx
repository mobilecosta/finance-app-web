...
import { Loader, AlertCircle, CheckCircle, Plus, Trash2, Edit2, Search, FileText, X } from 'lucide-react';
    ...
    <div className="card-hover">
      <div className="flex items-start justify-between">
        <div className="space-y-1 flex-1 min-w-0">
          <h3 className="font-medium text-zinc-100 truncate">{empresa.nome_razao_social}</h3>
          <p className="text-xs text-zinc-500 font-mono">{empresa.cpf_cnpj}</p>
          {empresa.nome_fantasia && <p className="text-xs text-zinc-500">{empresa.nome_fantasia}</p>}
          <p className="text-xs text-zinc-500">{empresa.email}</p>
          <p className="text-xs text-zinc-500">
            {empresa.endereco.cidade}/{empresa.endereco.uf}
          </p>
        </div>
        <div className="flex gap-1 flex-shrink-0">
          <button onClick={() => onConfig(empresa.cpf_cnpj)} className="p-1.5 hover:bg-zinc-800 rounded-lg transition-colors" title="Configurar NFS-e">
            <FileText className="w-4 h-4 text-blue-400" />
          </button>
          <button onClick={() => onEdit(empresa)} className="p-1.5 hover:bg-zinc-800 rounded-lg transition-colors" title="Editar">
            <Edit2 className="w-4 h-4 text-zinc-400" />
          </button>
          <button onClick={() => onDelete(empresa.cpf_cnpj)} className="p-1.5 hover:bg-zinc-800 rounded-lg transition-colors" title="Excluir">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500">Gerencie empresas e configurações na ACBr API</p>
        </div>
        {activeTab === 'lista' && (
      ...
      {loading && (
        <div className="flex justify-center p-8"><Loader className="w-6 h-6 animate-spin text-zinc-400" /></div>
      )}

      {!loading && activeTab === 'lista' && (
        <div className="space-y-4">
          <div className="flex gap-2">
            <input className="input max-w-xs font-mono text-sm" placeholder="Filtrar por CPF/CNPJ"
              value={filterCnpj} onChange={e => setFilterCnpj(e.target.value)} />
          ...
          <div className="space-y-2">
            {empresas.map(emp => (
              <EmpresaCard key={emp.cpf_cnpj} empresa={emp} onEdit={handleEdit} onDelete={setDeleteConfirm} onConfig={handleLoadConfig} />
            ))}
            {empresas.length === 0 && <p className="text-sm text-zinc-500 text-center py-8">Nenhuma empresa encontrada</p>}
          </div>
        </div>
      )}
      ...
      {!loading && activeTab === 'cadastrar' && (
        <div className="card max-w-2xl space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Inscrição Municipal</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Inscrição Estadual</label>
          ...
          <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider pt-2">Endereço</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-sm font-medium text-zinc-400 mb-2">Logradouro</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Número</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Complemento</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">CEP</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Código Município (IBGE)</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">UF</label>
      ...
      {!loading && activeTab === 'config' && configCnpj && (
        <div className="card max-w-lg space-y-4">
            ...
            <div className="flex gap-2">
              {(['homologacao', 'producao'] as const).map(a => (
                <button key={a} onClick={() => setConfig(c => ({ ...c, ambiente: a }))}
                  className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all duration-200 ${
                    config.ambiente === a ? 'bg-white text-black shadow-sm' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'
                  }`}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Lote RPS</label>
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Série RPS</label>
              <input className="input text-sm" value={config.rps.serie} onChange={e => setConfig(c => ({ ...c, rps: { ...c.rps, serie: e.target.value } }))} />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-2">Número RPS</label>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Simples Nacional</label>
              ...
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Regime Apuração</label>
              ...
              <div>
                <label className="block text-xs text-zinc-500 mb-1">Regime Especial</label>
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-400 mb-2">Incentivo Fiscal</label>
            <div className="flex gap-2">
              {[true, false].map(v => (
                <button key={String(v)} onClick={() => setConfig(c => ({ ...c, incentivo_fiscal: v }))}
                  className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all duration-200 ${
                    config.incentivo_fiscal === v ? 'bg-white text-black shadow-sm' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'
                  }`}
                >{v ? 'Sim' : 'Não'}</button>
              ))}
            </div>
        ...
        <div className="modal-overlay" onClick={() => setDeleteConfirm(null)}>
          <div className="modal-content max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setDeleteConfirm(null)} className="p-1 hover:bg-zinc-800 rounded-lg transition-colors">
                <X className="w-5 h-5 text-zinc-500" />
              </button>
            </div>
            <p className="text-sm text-zinc-400 mb-6">
              Tem certeza que deseja excluir a empresa <strong className="text-zinc-300">{deleteConfirm}</strong>? Esta ação não pode ser desfeita.
            </p>
            <div className="flex gap-2">
              <button onClick={() => handleDeletar(deleteConfirm)} className="flex-1 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors font-medium text-sm">
                Excluir
              </button>
